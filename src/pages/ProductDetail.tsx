    ...
    <div className="max-w-[1200px] mx-auto px-5 py-10">
      ...
      <div className="grid grid-cols-2 gap-10">
        <div className="text-center">
          <img src={product.image} alt={product.title} className="max-w-full h-[400px] object-contain" />
        </div>
        <div className="flex flex-col gap-4">
          <div className="flex justify-between items-center">
            <h1 className="text-[28px] text-gray-800">{product.title}</h1>
            <button onClick={handleFavoriteToggle} className="bg-none border-none text-[28px] cursor-pointer p-2 transition-transform duration-200 hover:scale-110" style={{ color: isInWishlist ? '#ff4444' : '#cccccc' }}>
              {isInWishlist ? '❤️' : '🤍'}
            </button>
          </div>
          <p className="text-base text-gray-500 capitalize">Category: {product.category}</p>
          <div className="text-base text-amber-400">Rating: {product.rating.rate} ★ ({product.rating.count} reviews)</div>
          ...
          <div className="flex items-center gap-2.5">
            ...
            <input type="number" min="1" value={quantity} onChange={(e) => setQuantity(parseInt(e.target.value) || 1)} className="w-[60px] p-2 text-base border border-gray-300 rounded-md focus:outline-none focus:border-blue-400" />
          </div>
          <button onClick={handleAddToCart} className="bg-blue-400 text-white border-none py-3.5 text-lg rounded-md cursor-pointer mt-5 transition-colors duration-300 hover:bg-blue-500">
            Add to Cart
          </button>
        </div>
      </div>
